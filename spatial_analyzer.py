...
class SpatialAnalyzer:
    ...
    def _convert_2d_to_3d(self, x: int, y: int, depth: float = None) -> Tuple[float, float, float]:
        ...
        # 智能深度约束 - 输电线深度不能超过标定物深度（符合物理规律：输电线架设在输电塔上）
        # 保持输电线的相对深度变化，确保自然的视觉效果
        original_depth = depth
        if self.calibration_depth is not None and depth > self.calibration_depth:
            # 对于超过标定物深度的点，使用相对深度缩放策略
            # 保持点之间的相对深度比例，同时确保不超过标定物深度
            # 计算深度缩放因子
            depth = self.calibration_depth
            print(f"点({x},{y})深度 {original_depth} 超过标定物深度 {self.calibration_depth}，已调整为 {depth}")
        ...
        if self.fx is not None and self.fy is not None and self.cx is not None and self.cy is not None:
            ...
            y_rel = (y - self.image_height / 2) / focal_length
        ...
        try:
            ...
            if show_all_points and self.depth_map is not None:
                ...
                for x, y, depth in zip(x_indices, y_indices, valid_depths):
                    try:
                        ...
                        if point_3d:
                            ...
                            z_flipped = -z_3d
                            all_x.append(x_3d)
                            all_y.append(y_flipped)
                            all_z.append(z_flipped)
                    except Exception as e:
                        continue
                ...
                if all_x:
                    ...
                    ax.scatter(all_x, all_z, all_y, c='gray', s=1, alpha=0.1, marker='.', label='Background Points')
            
            # Set figure title
            ax.set_title('3D Space Point Cloud Visualization', fontsize=15)
            
            # Set axis labels: X and Y centered at 0, Z negative (original setting)
            ax.set_xlabel('X (meters)', fontsize=10)
            ax.set_ylabel('Z (meters)', fontsize=10)  # Z-axis negative (original)
            ax.set_zlabel('Y (meters)', fontsize=10)  # Y-axis centered at 0, range -140 to 140
            ...
            for obj in self.danger_objects:
                if obj.get('center_3d') is not None:
                    ...
                    # 翻转Y坐标使y轴以0为中心对称，恢复Z坐标为负数（原始设置）
                    y_flipped = -y  # Y-axis centered at 0, same as X
                    z_flipped = -z   # Z-axis negative (original)
                    
                    danger_x.append(x)
                    danger_y.append(y_flipped)
                    danger_z.append(z_flipped)
                    
                    # Draw hazard point and add label with flipped coordinates
                    ax.scatter(x, z_flipped, y_flipped, c='red', s=100, marker='o', label=f'Hazard {obj["id"]}' if len(danger_x) <= 1 else "")
                    # Add text label
                    ax.text(x, z_flipped, y_flipped, f'Object{obj["id"]}', fontsize=8)
                ...
                if 'points_3d' in line and line['points_3d']:
                    ...
                    for point_info in line['points_3d']:
                        if point_info.get('3d') is not None:
                            ...
                            line_x.append(x)
                            line_y.append(y_flipped)
                            line_z.append(z_flipped)
                            valid_points.append(point_info)
                    
                    # Draw power line segments with flipped coordinates
                    if len(line_x) >= 2:
                        ax.plot(line_x, line_z, line_y, color=line_color, linewidth=2, 
                                label=f'Power Line {line.get("id", line_idx)}' if line_count <= 1 else "")
                        
                        # Draw power line points
                        ax.scatter(line_x, line_z, line_y, c=line_color, s=30, marker='^')
    ...
    def create_combined_visualization(self, image_path: str, distance_results: Dict, 
                                     output_path: str = None, show_plot: bool = False) -> str:
        """
