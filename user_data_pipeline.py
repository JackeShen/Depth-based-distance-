...
def generate_depth_map(image_path):
    ...
    try:
        # 初始化深度估计器
        estimator = DepthEstimator(encoder='vits')  # 使用轻量级编码器
        
        # 生成深度图
        depth_map = estimator.predict_depth(image_path, save=False)
        ...
        default_depth = np.ones((height, width)) * 5.0
        print("使用默认深度图")
        return default_depth

def calculate_distances(depth_map, danger_objects, segmentation_path, scale_factor, camera_matrix=None, fx=None, fy=None, cx=None, cy=None, regions_json_path=None, enable_smoothing=True, calibration_class_id=None, calibration_distance=None, distance_method='segment'):
    """
...
def visualize_with_user_data(image_path, analyzer, results, output_dir="user_results", calibration_class_id=None, calibration_distance=None, scale_factor=None, show_all_points_in_3d=False, max_all_points=10000):
    """
    ...
    try:
        ...
        for obj in analyzer.danger_objects:
            ...
            if is_calibration_object:
                ...
                marker_color = 'ro'
            ax.plot(center_x, center_y, marker_color, markersize=5)
            ...
            for dist_info in results['min_distances']:
                if dist_info['danger_object_id'] == obj['id']:
                    ...
                    if is_same_category_as_calibration and calibration_distance is not None:
                        ...
                        # For normal hazardous objects, display calculated distance and draw shortest distance line
                        ax.text(center_x + 5, center_y - 5, f"{distance:.1f}m", 
                        ...
                        danger_center = np.array([center_x, center_y])
                        ...
                        ax.plot([center_x, closest_point[0]], [center_y, closest_point[1]], 'b--', linewidth=2, alpha=0.8, label='Min Distance' if obj['id'] == '0' else "")
                    break
...
def main():
    ...
    # 用户需要修改以下路径为自己的文件路径
    user_config = {
        'image_path': r"/Users/oldshen/Desktop/3/原数据/3.jpg",          # 用户的图像文件
        'annotation_path': r"/Users/oldshen/Desktop/3/原数据/3.txt", # YOLO格式的txt标注
        'segmentation_path': r"/Users/oldshen/Desktop/3/原数据/3.json", # JSON格式的分割标注
        'depth_map_path': r'/Users/oldshen/PycharmProjects/deepanything/Marigold-main/output/in-the-wild_example/depth_npy/08d119836973085829e8bac9a350c01c_depth.npy',  # 如果有现成的深度图，可以提供路径
        'regions_json_path': None,  # 区域划分JSON文件路径（可选，用于区域化深度平滑）
        'enable_smoothing': False,   # 是否启用输电线深度平滑功能（可选，默认启用）
        'output_dir': r"/Users/oldshen/PycharmProjects/deepanything/deepview/Depth-Anything-V2-main/Depth-Anything-V2-main/Depth-Anything-V2-main/3Duser_results",  # 输出结果保存目录
        # 以下参数选择一种方式设置：
        'scale_factor': None,  # 手动设置比例因子（如果使用手动方式）
        'calibration_class_id': 1,  # 标定物的类别ID（如果使用标定物方式）
        'calibration_distance':500, # 标定物到摄像头的实际距离（米）（如果使用标定物方式）
        'show_all_points_in_3d': True,  # 是否显示所有点云（可选，默认显示）
        'max_all_points': 100000,  # 最大显示点云数量（可选，默认10000）
